...
PathReader::PathReader(const asset& tarball) {
...
asset& PathReader::operator[](const char* path_name) { return get(path_name); }

asset& PathReader::get(const char* path_name) {
    for (size_t i = 0; i < paths.size(); i++) {
        if (paths[i] == path_name) {
            pros::lcd::set_text(4, path_name);
            return assets[i];
        }
    }
...
bool PathReader::has(const char* path_name) { return std::find(paths.begin(), paths.end(), path_name) != paths.end(); }